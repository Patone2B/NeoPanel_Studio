...
// Données utilisateur : notes, météo, appareils autorisés et chemins d’applications.
...
function startServer() {
  ...
  app.get("/api/apps/config", (req, res) => {
    res.json({ ok: true, defaults: getDefaultApps(), appPaths: settings.appPaths || {} });
  });
  ...
  app.post("/api/apps/config", (req, res) => {
    ...
    res.json({ ok: true, appPaths: settings.appPaths });
  });
  ...
  app.post("/api/weather/settings", async (req, res) => {
    if (!isValidToken(req.body.token, req) && !isLocalRequest(req)) {
      return res.status(403).json({ ok: false, error: "Session invalide" });
    }

    const newApiKey = String(req.body.apiKey || "").trim();
    if (newApiKey) settings.weather.apiKey = newApiKey;

    settings.weather.city = String(req.body.city || "").trim() || settings.weather.city || "Calvi";
    ...
    res.json({
      ok: !result.error,
      configured: Boolean(settings.weather.apiKey),
      keyPreserved: !newApiKey && Boolean(settings.weather.apiKey),
      ...result
    });
  });
  ...
  app.post("/api/action", (req, res) => {
    const { token, action, appName, shortcut, audioDevice, path: appPath, keys } = req.body;
    ...
    try {
      ...
      if (action === "launch") result = launchApp(appName);
      if (action === "launchPath") result = launchPath(appPath);
      ...
      res.json({ ok: true, message: result?.message || "Action envoyée." });
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message });
    }
  });
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log(`Code de session : ${SESSION_CODE}`);
  });
}
...
function loadSettings() {
  try {
    ...
    return { ...structuredCloneSafe(defaultSettings), ...raw, weather: { ...defaultSettings.weather, ...(raw.weather || {}) }, alerts: { ...defaultSettings.alerts, ...(raw.alerts || {}) }, trustedDevices: raw.trustedDevices || {}, appPaths: raw.appPaths || {} };
  } catch {
    return structuredCloneSafe(defaultSettings);
  }
}
...
function launchApp(appName) {
  ...
  const app = defaults[appName];
  const target = configured || app?.path;
  const label = app?.label || appName || "Application";

  if (!target) {
    return { ok: false, error: `Application introuvable. Configure son chemin dans « Config apps » (${label}).` };
  }
  return launchPath(target, label);
}

function launchPath(target, label = "Application") {
  const clean = validateLaunchTarget(target);
  if (!clean.ok) {
    logEvent("security", clean.error || "Lancement refusé");
    return { ok: false, error: clean.error || "Lancement refusé." };
  }

  if (!fs.existsSync(clean.path)) {
    const message = `${label} introuvable. Configure son chemin dans « Config apps ».`;
    logEvent("apps", `${message} Chemin : ${clean.path}`);
  ...
  return { ok: true, message: `${label} lancé.` };
}
...
function isIpv4Interface(net) {
  const family = String(net?.family || "").toLowerCase();
...
function buildNetworkInfo() {
  ...
  for (const [name, list] of Object.entries(nets)) {
    for (const net of list || []) {
      if (!isIpv4Interface(net) || !net.address) continue;
      interfaces.push({
        name,
        address: normalizeIp(net.address),
        internal: Boolean(net.internal),
        private: isPrivateIpv4(net.address),
        mac: net.mac || "",
        cidr: net.cidr || ""
      });
    }
  }
  interfaces.sort((a, b) => Number(a.internal) - Number(b.internal) || Number(b.private) - Number(a.private) || a.name.localeCompare(b.name));
...
function getLanIp() {
  ...
  for (const [name, list] of Object.entries(nets)) {
    for (const net of list || []) {
      if (!isIpv4Interface(net) || net.internal || !net.address) continue;
      const address = normalizeIp(net.address);
      candidates.push({ name, address, private: isPrivateIpv4(address), linkLocal: address.startsWith("169.254.") });
    }
  }

  const preferred = candidates.find(item => item.private && !item.linkLocal);
  if (preferred) return preferred.address;
  const fallback = candidates.find(item => !item.linkLocal) || candidates[0];
  return fallback?.address || "127.0.0.1";
}
